<template>
  <div>
    <div id="nav-bar" class="nav">
      ...
      <span class="name-parent">终端管理</span>
    </div>
    <div class="msgManagement-content">
      <div class="msgManagement-content-top" style="height: 50px; ">
        ...
        <div class="search-box-right" style="float: right;">
          <el-input placeholder="请输入终端名称" v-model="input" clearable @keydown.enter.native="getList"></el-input>
      ...
      <div class="gridBox-table" style="width: 100%; margin: 15px auto;  ">
        ...
        >
          ...
          <el-table-column prop="name" label="终端名称" fit></el-table-column>
          <el-table-column prop="type" label="终端类型" fit></el-table-column>
          <el-table-column prop="cardNum" label="终端卡号" fit></el-table-column>
    ...
    <el-dialog title="终端管理操作" :visible.sync="dialogFormVisible" style="width:1000px;margin:0 auto;">
      <el-form :model="form" :rules="rules" ref="form">
        <el-form-item label="终端名称" :label-width="formLabelWidth" prop="terminalName">
          <el-input v-model="form.terminalName" autocomplete="off" style="width:300px;" clearable></el-input>
        </el-form-item>
        ...
        <el-form-item label="北斗卡号" :label-width="formLabelWidth" prop="cardNum">
        ...
        <el-form-item label="是否显示位置" :label-width="formLabelWidth" prop="position">
          <el-select v-model="form.position" placeholder="请选择">
            <el-option label="显示位置" value="1"></el-option>
            <el-option label="不显示位置" value="0"></el-option>
          </el-select>
        </el-form-item>
...
import { terminalList } from "network/mainRuequest";
import { terminalDel } from "network/mainRuequest";
import { terminalAdd } from "network/mainRuequest";
import { terminalEdit } from "network/mainRuequest";
import { terminalselect1 } from "network/mainRuequest";
import { terminalselect2 } from "network/mainRuequest";
import { terminalselect3 } from "network/mainRuequest";
import { terminalselect4 } from "network/mainRuequest";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        terminalName: [
          {  pattern: /^[^`!@#$%^&*.<>]+$/,required: true, message: "请填写正确的终端名称", trigger: "blur" }
        ],
        province: [{ required: true, message: "请选择省", trigger: "blur" }],
        cardNum: [
          {  pattern: /^[0-9]*$/, required: true, message: "请输入正确的北斗卡号", trigger: "change,blur" }
        ],
        industry: [
  ...
  methods: {
    ...
    addList() {
      let params = {
        name: this.form.terminalName,
        positionState: this.form.position,
    ...
    judgeedit() {
      ...
      if (this.isEdit == true) {
        if (this.selectList.length != 1) {
          ...
          this.form.terminalName = this.selectList[0].name;
          // this.form.position = this.selectList[0].type;
    ...
    select1() {
      let params = {};
      terminalselect1(params).then(res => {
    ...
    getCitySelect() {
      ...
      terminalselect2(params).then(res => {
    ...
    getCountySelect() {
      ...
      terminalselect3(params).then(res => {
        this.selectCounty = res.data.data;
      });
    },
    select4() {
      let params = {};
      terminalselect4(params).then(res => {
        if (res.data.success == false) {
